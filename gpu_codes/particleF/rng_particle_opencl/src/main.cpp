...
cl_kernel, cl_mem memObjectsMoveKernel[6]);
...
for(int topIndex=0; topIndex<iterations; topIndex++) 
{
	...
	move_rob(&myrobot, 0.6, 6.0, world_size);
	measurement << sense_rob(&myrobot, landmarks);
	...
	cl_mem memObjectsMoveKernel[6] = {0,0,0,0,0,0};
	...
	if (!CreateMemObjectsMoveKernel(context, memObjectsMoveKernel, 
	    	particle, measurement.data(), landmarks.data(), &move_params))
	{
	    CleanupMoveKernel(context, commandQueue, program, 
	    	kernel, memObjectsMoveKernel);
	    return 1;
...
//	    sizeof(cl_mem),(void *)&memObjectsMoveKernel[10]);
...
//	    sizeof(cl_int),(void *)&landcols);
	...
	if((int)(NUM_PARTICLES/lws)*lws < NUM_PARTICLES)
	{
		globalWorkSize[0] = lws * ((int)(NUM_PARTICLES/lws) + 1);
	}
	else
	{
...
//	if (errNum != CL_SUCCESS)
//	{
//	     cerr << "Error reading result buffer 'x_rng_out'." <<  endl;
//	    CleanupMoveKernel(context, commandQueue, program, 
//	    	kernel, memObjectsMoveKernel);
//	    return 1;
//	}
//   
//   FILE* file_rng = fopen("rng_out.txt", "a"); 
//	char output_rng[255]; 
//	for (int i=0; i<N; i++)
//	{
//	 	sprintf(output_rng, "%10.9f\n", x_rng_out[i]);
//		fputs(output_rng, file_rng); 
//	}
//	
//	fclose(file_rng); 
...
bool CreateMemObjectsMoveKernel(cl_context context, 
	cl_mem memObjectsMoveKernel[6], Robot *particle, 
...
//    memObjectsMoveKernel[10] = clCreateBuffer(context, 
//	CL_MEM_READ_WRITE,
//	sizeof(ulong) * NUM_PARTICLES, NULL, NULL);
