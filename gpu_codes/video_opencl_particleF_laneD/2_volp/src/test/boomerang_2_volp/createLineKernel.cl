                               ...
                               __global linestruct *line_output)
{
   ...
   line_output[ln].theta1 = ln * (M_PI_F)/cline_params->NUM_LINES;
	line_output[ln].theta1 = cyclicWorld((float)line_output[ln].theta1, (float)(M_PI_F));
   
   float angVar = (xadd[ln]/cline_params->imgcols)*(M_PI_F/6);

   line_output[ln].theta2 = ln * (M_PI_F)/cline_params->NUM_LINES - angVar;
	line_output[ln].theta2 = cyclicWorld((float)line_output[ln].theta2, (float)(M_PI_F));
	
   for(int l=0; l<cline_params->imgrows; l++)
	{
      line_output[ln].y[l] = l; 

      if(l <= (int)(cline_params->imgrows/4))
      {
         r = line_output[ln].y[l]/sin(line_output[ln].theta1);	
         line_output[ln].x[l] = fabs((xadd[ln] + r * (cos(line_output[ln].theta1))));
         line_output[ln].x[l] = cyclicWorld((float)(line_output[ln].x[l]), (float)(cline_params->imgcols));
      }
	   else
      {
         r = (line_output[ln].y[l]-line_output[ln].y[(int)(cline_params->imgrows/4)])/sin(line_output[ln].theta2);
         line_output[ln].x[l] = fabs(line_output[ln].x[(int)(cline_params->imgrows/4)] + r * (cos(line_output[ln].theta2)));
         line_output[ln].x[l] = cyclicWorld((float)(line_output[ln].x[l]), (float)(cline_params->imgcols));
      }
