...
{
	line.theta1 = line.theta1 + turnAngle + rngturn;
	line.theta2 = line.theta2 + turnAngle + rngturn;
  ...
  // xincept = (cyclicWorld_kernel(xincept, imgcols/2);
   ...
   if(line.x[0] <= imgcols/2)
   {
     ...
     // line.theta1 = cyclicWorld_kernel(line.theta1, 50.0f*M_PI_F/180.0f) + 10.0f * M_PI_F/18.0f;
     // line.theta2 = cyclicWorld_kernel(line.theta2, 50.0f*M_PI_F/180.0f) + 10.0f * M_PI_F/18.0f;
   }
   else
   {
      xincept = cyclicWorld_kernel(xincept-imgcols/2, imgcols/2) + (float)imgcols/2.0f; 
	  // line.theta1 = cyclicWorld_kernel(line.theta1, 50.0f*M_PI_F/180.0f) + M_PI_F/6.0f;
	  // line.theta2 = cyclicWorld_kernel(line.theta2, 50.0f*M_PI_F/180.0f) + M_PI_F/6.0f;
   }
	...
	for(int l=0; l<imgrows; l++)
	{
      ...
      if(l <= (int)(imgrows/4))
      {
		   r = line.y[l]/sin(line.theta1);	
		   line.x[l] = abs((int)(xincept + r * (cos(line.theta1))));
      }
	   else
      {
         r = (line.y[l]-line.y[(int)(imgrows/4)])/sin(line.theta2);
         line.x[l] = fabs(line.x[(int)(imgrows/4)] + r * (cos(line.theta2)));
      }
...
{       
    ...
    {	
	    prob_dist = gaussian_kernel(good_line[idx_dim1].theta1, allnoises->turnNoise,
         best_line[gl].theta1);	
	    prob_dist *= gaussian_kernel(good_line[idx_dim1].theta2, allnoises->turnNoise,
         best_line[gl].theta2);	
    ...
    good_line_out[idx_dim1].theta1 = good_line[idx_dim1].theta1; 
    good_line_out[idx_dim1].theta2 = good_line[idx_dim1].theta2; 
