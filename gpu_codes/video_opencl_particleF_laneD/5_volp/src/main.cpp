...
{
   system("rm testconditions.txt");
   system("rm best_data.txt");
   ...
   int mode = 3;
   int NUM_GOOD_LINES = 150;
//   int iterations = 1;
   int NUM_LINES = 30000;
   int NEIGHBORHOOD = 3;

   FILE* file_tc = fopen("testconditions.txt", "a"); 
   char output_tc[255]; 
   sprintf(output_tc, "mode = %d\n", mode);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "NUM_LINES = %d\n", NUM_LINES);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "NUM_GOOD_LINES = %d\n", NUM_GOOD_LINES);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "NUM_BEST_LINES = %d\n", NUM_BEST_LINES);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "BEST_LINE_DIST = %d\n", BEST_LINE_DIST);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "TURNMAX = %3.2f\n", TURNMAX);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "DISTMAX = %3.2f\n", DISTMAX);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "NEIGHBORHOOD = %d\n", NEIGHBORHOOD);
   fputs(output_tc, file_tc); 
   sprintf(output_tc, "Lane detection is done every %d-th frame\n", Iframes);
   fputs(output_tc, file_tc); 
   fclose(file_tc); 
   ...
   float total_clocks = 0.0; 
      ...
      FILE* file_bd = fopen("best_data.txt", "a"); 
      char output_bd[255]; 
      sprintf(output_bd, "%s     %5s   %12s        %10s     %10s     %5s   %5s   %5s\n", "fNo.", "mw1", "mw2", "best1.x[0]", "best1.theta", "best2.x[0]", "best2.theta", "t/frame");
      fputs(output_bd, file_bd); 
      fclose(file_bd); 
   ...
   for(int framme=0; framme < num_frames-4; framme++)
   {
      ...
      file_bd = fopen("best_data.txt", "a"); 
      sprintf(output_bd, "%d     ", framme);
      fputs(output_bd, file_bd); 
      fclose(file_bd); 
      ...
      float ROI_START_X_criteria = 0.10;
      float ROI_START_Y_criteria = 0.70;
      float xROI_criteria = 0.30;  
      float yROI_criteria = 1.00;  

      int ROI_START_X = (int)(ROI_START_X_criteria * img_org_full.cols);// % of image cropped in the right
      int ROI_START_Y = (int)(ROI_START_Y_criteria * img_org_full.rows);// % of image cropped in the top
      int xROI = img_org_full.cols - (int)(xROI_criteria * img_org_full.cols);
      int yROI = img_org_full.rows - yROI_criteria * ROI_START_Y;
      rect = cv::Rect(ROI_START_X, ROI_START_Y, xROI, yROI);
     
     if(framme == 0){ 
      file_tc = fopen("testconditions.txt", "a"); 
      sprintf(output_tc, "ROI_START_X_criteria = %3.2f\n", ROI_START_X_criteria);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "ROI_START_Y_criteria = %3.2f\n", ROI_START_Y_criteria);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "xROI_criteria = %3.2f\n", xROI_criteria);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "yROI_criteria = %3.2f\n", yROI_criteria);
      fputs(output_tc, file_tc); 
      fclose(file_tc); 
     }
      ...
      float GOOD_LINE_DIST = (float)img_grad.cols/(4.0*(float)NUM_GOOD_LINES);
     
     if(framme == 0){ 
      file_tc = fopen("testconditions.txt", "a"); 
      sprintf(output_tc, "GOOD_LINE_DIST = %e\n", GOOD_LINE_DIST);
      fputs(output_tc, file_tc); 
      fclose(file_tc); 
     } 
      ...
      if(framme == 0){ 
       file_tc = fopen("testconditions.txt", "a"); 
       sprintf(output_tc, "imgrows = %d\n", img_grad.rows);
       fputs(output_tc, file_tc); 
       sprintf(output_tc, "imgcols = %d\n", img_grad.cols);
       fputs(output_tc, file_tc); 
       fclose(file_tc); 
      } 
      ...
      if(total_v_frames%Iframes == 0)
      {
         ...
         if(framme == 0){ 
          file_tc = fopen("testconditions.txt", "a"); 
          sprintf(output_tc, "sizeof(createLineParams) = %zd Bytes\n", sizeof(struct createLineParams));
          fputs(output_tc, file_tc); 
          fclose(file_tc); 
         } 
         //-----------
         ...
         if(framme == 0){ 
          file_tc = fopen("testconditions.txt", "a"); 
          sprintf(output_tc, "sizeof(img_grad_array) = %ld Bytes\n", img_grad.rows*img_grad.cols*sizeof(int));
          fputs(output_tc, file_tc); 
          fclose(file_tc); 
         } 
         //-----------
     ...
     if(framme == 0){ 
      file_tc = fopen("testconditions.txt", "a"); 
      sprintf(output_tc, "%s\n", "Kernel2:");
      sprintf(output_tc, "localWorkSize = %zd\n", localWorkSize[0]);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "globalWorkSize = %zd\n", globalWorkSize[0]);
      fputs(output_tc, file_tc); 
      fclose(file_tc); 
     } 
         ...
         if(framme == 0){ 
          file_tc = fopen("testconditions.txt", "a"); 
          sprintf(output_tc, "N*sizeof(line_new) = %ld Bytes\n", NUM_LINES * sizeof(struct lineStruct));
          fputs(output_tc, file_tc); 
          fclose(file_tc); 
         } 
        ...
        cout << "shalf = " << shalf << endl;
         ...
         float dummy_mw = 0.0;
         sprintf(output_bd, "%10.9f     %10.9f     ", dummy_mw, dummy_mw);
         fputs(output_bd, file_bd); 
         ...
         sprintf(output_bd, "%4.3f     %4.3f    ", best_line[bl].x[0]+ROI_START_X, best_line[bl].theta*180.0/PI);
         fputs(output_bd, file_bd); 
      ...
      { //IF VIDEO FRAME > 0
         ...
         allnoises->senseNoise = 0.05;
         
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "forwardNoise = %f\n", allnoises->forwardNoise);
             fputs(output_tc, file_tc); 
             sprintf(output_tc, "turnNoise = %f\n", allnoises->turnNoise);
             fputs(output_tc, file_tc); 
             sprintf(output_tc, "senseNoise = %f\n", allnoises->senseNoise);
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
         ...
         //{
            ...
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "N*sizeof(good_lines) = %ld Bytes\n", NUM_GOOD_LINES * sizeof(struct lineStruct));
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
            //-----------
            ...
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "N*sizeof(best_lines) = %ld Bytes\n", NUM_BEST_LINES * sizeof(struct lineStruct));
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
           ...
           // }
            ...
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "sizeof(noiseStruct) = %ld Bytes\n", sizeof(struct noiseStruct));
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
            ...
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "sizeof(wtKernParams) = %ld Bytes\n", sizeof(struct wtKernParams));
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
     ...
     if(framme == 1){ 
      ...
      fclose(file_tc); 
            ...
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "N*sizeof(good_line_new) = %ld Bytes\n", NUM_GOOD_LINES * sizeof(struct lineStruct));
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
            //OUTPUT 2
            ...
            if(framme == 1){ 
             file_tc = fopen("testconditions.txt", "a"); 
             sprintf(output_tc, "N*sizeof(w_pf_new) = %ld Bytes\n", NUM_GOOD_LINES * sizeof(float));
             fputs(output_tc, file_tc); 
             fclose(file_tc); 
            } 
    ...
    if(mode == 2 || mode == 3){
      int NUM_HLINES = (int)img_gray.rows/2 - 2;
     
     if(framme == 0){ 
      file_tc = fopen("testconditions.txt", "a"); 
      sprintf(output_tc, "NUM_HLINES = %d\n", NUM_HLINES);
      fputs(output_tc, file_tc); 
      fclose(file_tc); 
     } 
         ...
         for(int hline=0; hline <= NUM_HLINES; hline++)
          {
             ...
             if(mode == 2){
                  //DRAW HLINES
                  std::vector< std::pair< std::pair<int, int>, std::pair<int, int> > > hline_visual;
                  int xh1=0, yh1=0, xh2=0, yh2=0;  
                  
                  xh1 =(int) (best_line[bl].x[hlidx]) - H_NEIGHBORHOOD; 
                  yh1 =(int) (best_line[bl].y[hlidx]);
                  xh2 =(int) (best_line[bl].x[hlidx]) + H_NEIGHBORHOOD; 
                  yh2 = yh1;
                  
                  hline_visual.push_back(std::pair< std::pair<int, int>, std::pair<int, int> >
                  (std::pair<int, int>( xh1+ROI_START_X, yh1+ROI_START_Y ), std::pair<int, int>( xh2+ROI_START_X, yh2+ROI_START_Y )));  
      
                  std::vector< std::pair< std::pair<int, int>, std::pair<int, int> > >::iterator it_hline;  
                  for(it_hline=hline_visual.begin();it_hline!=hline_visual.end();it_hline++)  
                  {  
                       cv::line(img_org_full, cv::Point(it_hline->first.first, it_hline->first.second), 
                       cv::Point(it_hline->second.first, it_hline->second.second), cv::Scalar(77,153,255), 1, 8,0);  
                  }
                  //DRAW HLINES
               }
    ...
    file_bd = fopen("best_data.txt", "a"); 
    sprintf(output_bd, "%f\n", diff_total/(float)CLOCKS_PER_SEC);
    fputs(output_bd, file_bd); 
    fclose(file_bd); 
   ...
   //fclose(file_rt); 
    ...
    cout<<"Total_clocks = " << total_clocks << "\n"<< endl;
      ...
      file_tc = fopen("testconditions.txt", "a"); 
      sprintf(output_tc, "CLOCKS_PER_SEC = %ld\n", CLOCKS_PER_SEC);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "Total_clocks = %f\n", total_clocks);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "Total_frames = %f\n", num_frames);
      fputs(output_tc, file_tc); 
      sprintf(output_tc, "Average Frames/second = %f\n", (CLOCKS_PER_SEC * num_frames)/total_clocks);
      fputs(output_tc, file_tc); 
      fclose(file_tc); 
