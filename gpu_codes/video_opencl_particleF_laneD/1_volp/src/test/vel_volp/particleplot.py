...
data = np.loadtxt('goodlines.txt')
data1 = np.loadtxt('bestlines.txt')
...
for i in range(100):
	...
	ax = plt.subplot(10,10,i+1, axisbg='white')
	
	plt.plot(data[ i*NUM_GOOD_LINES : (i+1)*NUM_GOOD_LINES , 0 ], data[  i*NUM_GOOD_LINES : (i+1)*NUM_GOOD_LINES, 1], 'cD', markersize=4)
	#plt.plot(data1[(i*NUM_BEST_LINES):((i+1)*NUM_BEST_LINES-1), 0], data1[(i*NUM_BEST_LINES):((i+1)*NUM_BEST_LINES-1), 1],'rs', markersize=8)
###	plt.plot(data1[i*NUM_BEST_LINES:(i+1)*NUM_BEST_LINES, 0], data1[i*NUM_BEST_LINES:(i+1)*NUM_BEST_LINES, 1],'rs', markersize=8)
