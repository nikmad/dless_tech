...
int main( int argc, char** argv )
{
   ...
   string input_video_file = "../../../../1_dependencies/video/clip5"; 
...
//TEMP      int NUM_HLINES = 50;
//TEMP      int H_NEIGHBORHOOD_BASE = 5;
//TEMP      //int color_hline[] = {0,102,255};//ORANGE
//TEMP      int color_hline[] = {204,0,51};//BLUE
//TEMP      for(int bl=0; bl<NUM_BEST_LINES; bl++)
//TEMP      {
//TEMP         int max_intensity_hline = 0;
//TEMP         for(int hline=0; hline <= NUM_HLINES; hline++)
//TEMP          {
//TEMP             //int H_NEIGHBORHOOD = 2 * (int)((float)H_NEIGHBORHOOD_BASE * ((float)hline/(float)H_NEIGHBORHOOD_BASE)) + 2;
//TEMP             int hlidx = hline * (img_gray.rows/NUM_HLINES); 
//TEMP             int H_NEIGHBORHOOD = 20;
//TEMP             if( y1 >= 0.30 )
//TEMP             {
//TEMP               H_NEIGHBORHOOD = 30;
//TEMP             }
//TEMP             for(int i=-1*(H_NEIGHBORHOOD); i<H_NEIGHBORHOOD; i++)
//TEMP             {
//TEMP                x1 = (int) ((best_line[bl].x[hlidx]) + i);
//TEMP                y1 = (int) (best_line[bl].y[hlidx]);
//TEMP                
//TEMP                if((int)img_gray_full.at<uchar>( y1+ROI_START_Y , x1+ROI_START_X ) > max_intensity_hline)
//TEMP                {
//TEMP                   max_intensity_hline = (int)img_gray_full.at<uchar>( y1+ROI_START_Y , x1+ROI_START_X );
//TEMP                }
//TEMP             }
//TEMP          }
//TEMP         for(int hline=0; hline <= NUM_HLINES; hline++)
...
//TEMP             }
//TEMP      
//TEMP             //DRAW HLINES std::vector< std::pair< std::pair<int, int>, std::pair<int, int> > > hline_visual;
//TEMP             //DRAW HLINES int xh1=0, yh1=0, xh2=0, yh2=0;  
//TEMP             //DRAW HLINES 
//TEMP             //DRAW HLINES xh1 =(int) (best_line[bl].x[hlidx]) - H_NEIGHBORHOOD; 
//TEMP             //DRAW HLINES yh1 =(int) (best_line[bl].y[hlidx]);
//TEMP             //DRAW HLINES xh2 =(int) (best_line[bl].x[hlidx]) + H_NEIGHBORHOOD; 
//TEMP             //DRAW HLINES yh2 = yh1;
//TEMP             //DRAW HLINES 
//TEMP             //DRAW HLINES hline_visual.push_back(std::pair< std::pair<int, int>, std::pair<int, int> >
//TEMP             //DRAW HLINES (std::pair<int, int>( xh1+ROI_START_X, yh1+ROI_START_Y ), std::pair<int, int>( xh2+ROI_START_X, yh2+ROI_START_Y )));  
//TEMP      
//TEMP             //DRAW HLINES std::vector< std::pair< std::pair<int, int>, std::pair<int, int> > >::iterator it_hline;  
//TEMP             //DRAW HLINES for(it_hline=hline_visual.begin();it_hline!=hline_visual.end();it_hline++)  
//TEMP             //DRAW HLINES {  
//TEMP             //DRAW HLINES      cv::line(img_org_full, cv::Point(it_hline->first.first, it_hline->first.second), 
//TEMP             //DRAW HLINES      cv::Point(it_hline->second.first, it_hline->second.second), cv::Scalar(77,153,255), 1, 8,0);  
//TEMP             //DRAW HLINES }
//TEMP             
//TEMP             for(int i=-1*(H_NEIGHBORHOOD); i<H_NEIGHBORHOOD; i++)
//TEMP             {
...
//TEMP                y1 = (int) (best_line[bl].y[hlidx]);
//TEMP                
//TEMP                //cout << "value of "<< i << ": " << (int)img_gray_full.at<uchar>( y1+ROI_START_Y, x1 ) << endl;
//TEMP                
//TEMP                //if( y1 >= 0.30 * img_gray.rows && (int)img_gray_full.at<uchar>( y1+ROI_START_Y , x1+ROI_START_X ) > 0.75 * max_intensity_hline)//day
//TEMP                if( y1 >= 0.30 * img_gray.rows && (int)img_gray_full.at<uchar>( y1+ROI_START_Y , x1+ROI_START_X ) > 0.70 * max_intensity_hline)
//TEMP                {
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y , x1+ROI_START_X )[0]   = color_hline[0]; 
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y , x1+ROI_START_X )[1]   = color_hline[1]; 
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y , x1+ROI_START_X )[2]   = color_hline[2]; 
//TEMP                   
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y+1 , x1+ROI_START_X )[0] = color_hline[0]; 
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y+1 , x1+ROI_START_X )[1] = color_hline[1]; 
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y+1 , x1+ROI_START_X )[2] = color_hline[2]; 
//TEMP                }
//TEMP                //else if( y1 < 0.30 * img_gray.rows && (int)img_gray_full.at<uchar>( y1+ROI_START_Y , x1+ROI_START_X ) > 0.75 * max_intensity_hline)//day
//TEMP                else if( y1 < 0.30 * img_gray.rows && (int)img_gray_full.at<uchar>( y1+ROI_START_Y , x1+ROI_START_X ) > 0.40 * max_intensity_hline)
//TEMP                {
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y , x1+ROI_START_X )[0]   = color_hline[0]; 
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y , x1+ROI_START_X )[1]   = color_hline[1]; 
//TEMP                   img_org_full.at<cv::Vec3b>( y1+ROI_START_Y , x1+ROI_START_X )[2]   = color_hline[2]; 
