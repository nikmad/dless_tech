...
//	ULONG N = (int)(NUM_LINES/30);
	...
	{	
		...
		//move_line(&goodlinepf[gl], turn1, dist1, img_grad.rows, img_grad.cols);
		//move_line(&goodlinepf[gl], TURNMAX, DISTMAX, img_grad.rows, img_grad.cols);
		//SENSING: 
	//	int NEIGHBORHOOD = 5;
	...
	for(int topIndex=0; topIndex<iterations; topIndex++) 
	{
		for(int bl=0; bl<NUM_BEST_LINES; bl++) //'gl' for goodline
		{	
			bestlinepf[bl].linvel = LIN_VEL;
			bestlinepf[bl].angvel = ANG_VEL;

         //move_line(&bestlinepf[bl], 0.1, 0.5, img_grad.rows, img_grad.cols, FRAME_RATE);
			move_line(&bestlinepf[bl], img_grad.rows, img_grad.cols, FRAME_RATE);
			//SENSING: 
			for(int blord=0;blord<img_grad.rows;blord++)//glord for goodline coordinate
			{
			RNG rng(time(NULL));
      ...
      for(int i=0; i<NUM_GOOD_LINES; i++)
      {
         ...
         // dist1 = ((float)c2/10000.0000)*DISTMAX;
		    //move_line(&goodlinepf[i], turn1, dist1, img_grad.rows, img_grad.cols);
			 goodlinepf[i].linvel = bestlinepf[0].linvel;
			 goodlinepf[i].angvel = bestlinepf[0].angvel;
		    
          move_line(&goodlinepf[i], img_grad.rows, img_grad.cols, FRAME_RATE);
		    ...
		    //int gl = 0;
		    //w_pf[i] = gaussian(linepf[i].dist, linepf[i].senseNoise, goodlinepf[gl].dist);
		    ...
		    //cout << "linepf[i].dist = " << linepf[i].dist << endl;
		} //CLOSING i-LOOP.	
...
//			for(int gl=0; gl<NUM_BEST_LINES; gl++){	
//			//	prob_dist = gaussian(goodlinepf[i].theta, goodlinepf[i].turnNoise, bestlinepf[gl].theta);	
//			//	prob_dist = gaussian(goodlinepf[i].dist/goodlinepf[0].dist, goodlinepf[i].forwardNoise, bestlinepf[gl].dist/bestlinepf[0].dist);
//				prob_dist = gaussian(goodlinepf[i].x[0], 5.0, bestlinepf[gl].x[0]);
//				prob_dist *= gaussian(goodlinepf[i].x[img_grad.rows-1], 5.0, bestlinepf[gl].x[img_grad.rows-1]);
//				//cout << prob_dist << endl;
//				if( prob_dist > w_pf[i])
//				{
//				w_pf[i] = prob_dist;
