...
void move_line(linestruct *line, int imgrows, int imgcols, float fps) 
{
...
//	line->theta = line->theta + turnAngle + rng.gaussian(line->turnNoise);
	line->theta = line->theta + line->angvel*dt + rng.gaussian(line->turnNoise);
	line->theta = cyclicWorld(line->theta, PI);

	//moveDistance = moveDistance + rng.gaussian(line->forwardNoise);
	moveDistance = line->linvel*dt + rng.gaussian(line->forwardNoise);
