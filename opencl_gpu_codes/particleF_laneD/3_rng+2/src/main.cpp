...
//	}
   ...
   if (!CreateMemWt(context_wt, mem_wt, good_line, best_line, 
         img_grad_array, 
         //turnArray, distArray, rngturnArray, rngfwdArray, 
...
//   }
  ...
  //     sizeof(cl_mem),(void *)&mem_wt[6]);
   ...
   size_t lws = localWorkSize_wt[0];
	
	size_t globalWorkSize_wt[1] = { 0 };
	
	if((int)(NUM_GOOD_LINES/lws)*lws < NUM_GOOD_LINES)
	{
		globalWorkSize_wt[0] = lws * ((int)(NUM_GOOD_LINES/lws) + 1);
	}
	else
	{
		globalWorkSize_wt[0] = NUM_GOOD_LINES;
	}
