...
// 	system("rm bestcord.txt");
	//system("rm goodCord.txt");
	//system("rm bestCord.txt");
	system("rm goodlines.txt");
	system("rm bestlines.txt");
	system("rm out1.png");
	system("rm out2.png");
   ...
   FILE *filegood, *filebest;
   char output_good[255]; 
   char output_best[255]; 
   
   for(int framme=0; framme < num_frames-100; framme++)
   //for( ; ; )
   {
   ...
   //   cout << "________________________________________\n" << endl;
      ...
      //if(total_v_frames == 0)
      {
        ...
        // }
         ...
         {
             line_new[i].linvel = 0.0;
             line_new[i].angvel = 0.0;
         ...
         //Writing the OUTPUT FILE
         filegood = fopen("goodlines.txt", "a"); 
         
         for (int i=0; i<NUM_GOOD_LINES; i++)
         {
         sprintf(output_good, "%10.9f   %10.9f   %10.9f   %10.9f   %10.9f\n", 
         	good_line[i].x[0], good_line[i].theta, good_line[i].linvel, good_line[i].angvel, good_line[i].dist); 
         fputs(output_good, filegood); 
         }
         fclose(filegood);
         
         filebest = fopen("bestlines.txt", "a"); 
         //char output_best[255]; 
         
         for (int i=0; i<NUM_BEST_LINES; i++)
         {
         sprintf(output_best, "%10.9f   %10.9f   %10.9f   %10.9f   %10.9f\n", 
         	best_line[i].x[0], best_line[i].theta, best_line[i].linvel, best_line[i].angvel, best_line[i].dist); 
         fputs(output_best, filebest); 
         }
         fclose(filebest);
    ...
    //  else if(total_v_frames > 0)
      ...
      { //IF VIDEO FRAME > 0
         ...
         //   float angvel = 0.0;
            ...
            float blinex1 = best_line[0].x[0];
            float blinetheta1 = best_line[0].theta;
            //-----------
           if(total_v_frames == 1)
           {
              for(int gl=0; gl<NUM_GOOD_LINES; gl++)
              {
                  srand(gl*154350+20*gl*gl*gl);
            ...
            else if(total_v_frames > 1)
            {
            
              for(int gl=0; gl<NUM_GOOD_LINES; gl++)
              {
                  srand(gl*1247893+20*gl*gl*gl);

                  c1 = rand()%10000;
                  distArray[gl] = ((float)c1/10000.0000 - 0.5)*1.5*good_line[gl].linvel/FRAME_RATE;
                  
                  c2 = rand()%10000;
                  turnArray[gl] = ((float)c2/10000.0000 - 0.5)*1.5*good_line[gl].angvel/FRAME_RATE;
         ...
         //   best_line = new linestruct [NUM_BEST_LINES];
            ...
            float blinex2 = best_line[0].x[0];
            float blinetheta2 = best_line[0].theta;

            if(total_v_frames > 0)
            {
              for(int velidx=0; velidx<NUM_GOOD_LINES; velidx++)
              {
               good_line[velidx].linvel = (blinex2 - blinex1) * FRAME_RATE;
               good_line[velidx].angvel = (blinetheta2 - blinetheta1) * FRAME_RATE;
              }
            }
