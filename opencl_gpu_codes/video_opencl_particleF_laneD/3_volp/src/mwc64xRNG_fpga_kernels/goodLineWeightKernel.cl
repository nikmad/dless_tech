...
//void goodLineWeightKernel(__constant const struct lineStruct * restrict good_line,
                          __global const lineStruct * restrict best_line,
                         // __global const struct lineStruct * restrict best_line,
                          //__attribute__((local_mem_size(8192))) __local const struct lineStruct * restrict best_line,
                          __global const int * restrict img_grad,
                          //__global const int * restrict img_grad,
                          __constant noiseStruct * restrict allnoises,
                          //__attribute__((local_mem_size(32))) __local struct noiseStruct * restrict allnoises,
...
{       
   ...
   //rn_turn	
	...
	//rn_turn = (num/pow(10.0f, (float)(num_digits))) * turnmax - turnmax/3.0f;
	rn_turn = (num/pow(10.0f, (float)(num_digits))) * turnmax;
   //rn_dist
	num = MWC64X_NextUint(&rng);
	num_digits = findn(num);
	//rn_dist = (num/pow(10.0f, (float)(num_digits))) * distmax - distmax/3.0f;
   ...
   else
   {
      ...
      xincept = cyclicWorld_kernel(xincept-(float)params->imgcols/2.0f, 2.0*(float)params->imgcols/8.0f) + 5.0f*(float)params->imgcols/8.0f; 
