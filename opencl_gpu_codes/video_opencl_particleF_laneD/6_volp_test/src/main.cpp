...
int main( int argc, char** argv )
{
   ...
   int Iframes = 100;//Take accurate measurement after every Iframes.
...
//   int iterations = 1;
   ...
   for(int framme=0; framme < num_frames-4; framme++)
   {
      ...
      //for cordova1----------
      int ROI_START_X = (int)(0.30*img_org_full.cols);// % of image cropped in the right
      int ROI_START_Y = (int)(0.40*img_org_full.rows);// % of image cropped in the top
      int xROI = img_org_full.cols-(int)(0.60*img_org_full.cols);
      int yROI = 0.3*img_org_full.rows;
      ...
      //for Oli_lanes------------
      //int ROI_START_X = (int)(0.05*img_org_full.cols);// % of image cropped in the right
      //int ROI_START_Y = (int)(0.71*img_org_full.rows);// % of image cropped in the top
      //int xROI = img_org_full.cols-(int)(0.10*img_org_full.cols);
      //int yROI = img_org_full.rows-ROI_START_Y;
      //rect = cv::Rect(ROI_START_X, ROI_START_Y, xROI, yROI);
